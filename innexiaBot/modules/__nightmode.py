    ...
    for pro in chats:
        try:
            await tbot.send_message(
              int(pro.chat_id), "9pm, Group Is Closing Till 5pm. tomarrrrrrrrrrrrrroo !"
            )
            await tbot(
...
scheduler.add_job(job_close, trigger="cron", hour=20, minute=59)
scheduler.start()
...
async def job_open():
    ...
    for pro in chats:
        try:
            await tbot.send_message(
              int(pro.chat_id), "05:00 Am, Group Is Opening."
            )
            await tbot(
            functions.messages.EditChatDefaultBannedRightsRequest(
